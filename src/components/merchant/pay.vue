    ...
    <div class="pt-20" v-if="showAble">
      <div class="back-242">
        ...
        <div class="pl-30 pr-30">
          <div v-if="seatOrderNo && !(seatPayStatus&&!deductibleAmount)" class="height-90 pl-30 pr-30 flex-between fz-28 color-104 border-t-233 ">
            <div>订桌</div>
          ...
          <div v-for="item in selectedList" class="pay-item border-t-233">
            ...
            <div class="flex-right">
              ...
              <div class="price ">￥{{item.allPrice | moneyFilter}}</div>
            </div>
          </div>
        </div>
          ...
          <div class="notice pr-30 fz-24 color-104 text-right pt-24">
            ...
            !seatPayStatus?
              (allPrice>=lowCostAmount
                ?('提示:最低消费'+lowCostAmount.turnMoney()+',已包含桌位费')
                :('提示:最低消费'+lowCostAmount.turnMoney()+',剩余金额可于当前桌消费抵扣'))
              :(diffAmount?('提示:已抵扣￥'+diffAmount.turnMoney()+',' +(payPrice==0?'无需再付':('需另付￥'+payPrice.turnMoney()))):'')
            }}
          </div>
        </div>
