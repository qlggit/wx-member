<template>
  <div class="height-100-100 width-100-100 book-content pt-header">
    <header-component :header-data="{title:name}"></header-component>
      <div class="height-100-100 back-transparent overflow-scroll" @click="showConfirmWindow(1)">
      </div>
    <div class="position-fixed width-100-100 bottom-0 left-0 z-index-100 book-window pl-20 pr-20">
      <div class="flex-center">
        <div v-for="item in footerList" class="item text-center"  >
          <img :src="item.img">
          <div class="fz-30 color-24 break-none">{{item.name}}</div>
        </div>
      </div>
      <div class="pt-40">
        <div class="btn shadow-bottom-auto back-24 color-white">请选座位</div>
      </div>
    </div>
